        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
            ...
            <nav class="bg-white shadow-2xl rounded-2xl p-6 flex flex-col h-fit lg:h-[calc(100vh-8rem)] animate__animated animate__fadeInLeft">
                ...
                <form id="logout-form" action="{{ route('logout') }}" method="POST" class="mt-auto">
                    @csrf
                    <button type="submit" class="w-full text-left text-white font-semibold py-2 px-3 rounded-md bg-gradient-to-r from-red-500 to-pink-600 hover:from-pink-600 hover:to-red-500 transition duration-300">
                        Logout
                    </button>
                </form>
            </nav>
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm max-w-xl">
                        <form action="{{ route('enroll') }}" method="POST" class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="student_id" id="student_id" required class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-blue-500 py-2">
                            ...
                            <div>
                                ...
                                <select name="course_id" id="course_id" required class="block w-full rounded-md border-gray-300 focus:border-blue-500 focus:ring-blue-500 py-2">
                                    @foreach ($courses as $course)
                                        <option value="{{ $course->id }}">{{ $course->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <button type="submit" class="w-full text-center py-2 px-6 bg-gradient-to-r from-green-500 to-blue-500 text-white font-semibold rounded-md hover:from-blue-600 hover:to-green-600 transition duration-300">
                                Enroll Student
                            </button>
                        </form>
                    </div>
